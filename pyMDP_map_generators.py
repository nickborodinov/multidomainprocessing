...
class cross_corr_FFT_PCA:
    def __init__(self,image,n_components=50,window_size=20,step=5,display=False):
        self.n_components=n_components
        self.window_size=window_size
        self.step=step
        self.image=image
    ...
    def fit(self,components=[1,2,3]):
        self.pca_maps=np.zeros([len(components),self.image.shape[0],self.image.shape[1]])
